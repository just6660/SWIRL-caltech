...
from ssm.primitives import hmm_normalizer, arhmm_normalizer
from ssm.messages import hmm_expected_states, hmm_filter, hmm_sample, viterbi, arhmm_filter, arhmm_expected_states, arhmm_viterbi
...
class HMM(object):
    ...
    def __init__(self, K, D, M=0, init_state_distn=None,
        ...
        observation_classes = dict(
            ...
            mdpcategorical=obs.MDPCategoricalObservations,
...
class ARHMM(object):
    ...
    def expected_states(self, data, input=None, mask=None, tag=None):
        ...
        Ps = self.transitions.transition_matrices(data, input, mask, tag)
        log_likes = self.observations.log_likelihoods(data, input, mask, tag)
        return arhmm_expected_states(pi0, Ps, log_likes)
    ...
    def filter(self, data, input=None, mask=None, tag=None):
        ...
        Ps = self.transitions.transition_matrices(data, input, mask, tag)
        log_likes = self.observations.log_likelihoods(data, input, mask, tag)
        return arhmm_filter(pi0, Ps, log_likes)
    ...
    def _fit_em(self, datas, inputs, masks, tags, verbose = 2, num_iters=100, tolerance=0,
        ...
        for itr in pbar:
            ...
            # print(expectations[0][0])
            # print('fit em finish')

            # M step: maximize expected log joint wrt parameters
            # print('before m step')
            # print(self.init_state_distn.initial_state_distn)
            # print(self.transitions.transition_matrices(np.arange(26)[:, None], None, None, None))
            # # from scipy.special import logsumexp
            # ps = np.exp(self.observations.logits - logsumexp(self.observations.logits, axis=-1, keepdims=True))
            # print(ps)
            self.init_state_distn.m_step(expectations, datas, inputs, masks, tags, **init_state_mstep_kwargs)
            self.transitions.m_step(expectations, datas, inputs, masks, tags, **transitions_mstep_kwargs)
            self.observations.m_step(expectations, datas, inputs, masks, tags, **observations_mstep_kwargs)
            ...
            # Store progress
            # lls.append(self.log_prior() + sum([ll for (_, _, ll) in expectations]))
            lls.append(sum([ll for (_, _, ll) in expectations]))
...
class HSMM(HMM):
    ...
    def expected_states(self, data, input=None, mask=None, tag=None):
        ...
        Ps = self.transitions.transition_matrices(data, input, mask, tag)
        log_likes = self.observations.log_likelihoods(data, input, mask, tag)
        Ez, Ezzp1, normalizer = arhmm_expected_states(replicate(pi0, m), Ps, replicate(log_likes, m))
    ...
    def filter(self, data, input=None, mask=None, tag=None):
        ...
        Ps = self.transitions.transition_matrices(data, input, mask, tag)
        log_likes = self.observations.log_likelihoods(data, input, mask, tag)
        pzp1 = arhmm_filter(replicate(pi0, m), Ps, replicate(log_likes, m))
    ...
    def log_likelihood(self, datas, inputs=None, masks=None, tags=None):
        ...
        for data, input, mask, tag in zip(datas, inputs, masks, tags):
            ...
            Ps = self.transitions.transition_matrices(data, input, mask, tag)
            log_likes = self.observations.log_likelihoods(data, input, mask, tag)
            ll += arhmm_normalizer(replicate(pi0, m), Ps, replicate(log_likes, m))
            assert np.isfinite(ll)
        return ll
