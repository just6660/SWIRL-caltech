...
@numba.jit(nopython=True, cache=True)
def ar_forward_pass(pi0,
                 Ps,
                 log_likes,
                 alphas):

    T = log_likes.shape[0] + 1  # number of time steps
    K = log_likes.shape[1]  # number of discrete states
    ...
    for t in range(T-1):
        m = np.max(alphas[t])
        alphas[t+1] = np.log(np.dot(np.exp(alphas[t] - m), Ps[t * hetero])) + m + log_likes[t]
    return logsumexp(alphas[T-1])
...
@numba.jit(nopython=True, cache=True)
def arhmm_filter(pi0, Ps, ll):
    T, K = ll.shape

    # Forward pass gets the predicted state at time t given
    # observations up to and including those from time t
    alphas = np.zeros((T, K))
    ar_forward_pass(pi0, Ps, ll, alphas)
...
def hmm_expected_states(pi0, Ps, ll):
    ...
    # expected_states = np.exp(expected_states)[:-1]
    ...
    if not stationary:
        # expected_joints = alphas[:-1,:,None] + betas[1:,None,:] + ll[:,None,:] + log_Ps
        # print(alphas[:-1,:,None].shape, betas[1:,None,:].shape, ll[1:,None,:].shape, log_Ps.shape)
...
def arhmm_expected_states(pi0, Ps, ll):
    T, K = ll.shape
    ...
    if not stationary:
        ...
        # Compute the sum over time axis of the expected joints
        expected_joints = np.zeros((K, K))
        _compute_stationary_expected_joints(alphas[:-1], betas[1:], ll, log_Ps[0], expected_joints)
        expected_joints = expected_joints[None, :, :]
...
def arhmm_viterbi(pi0, Ps, ll):
    """
    Find the most likely state sequence
    """
    return _arhmm_viterbi(pi0, Ps, ll).astype(int)
...
@primitive
def arhmm_normalizer(pi0, Ps, ll):
    T, K = ll.shape
    alphas = np.zeros((T+1, K))
